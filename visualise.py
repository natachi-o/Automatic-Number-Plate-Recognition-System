...
for car_id in np.unique(results['car_id']):
    max_ = np.amax(results[results['car_id'] == car_id]['licence_number_score'])
    licence_plate[car_id] = {'licence_crop': None,
                             'licence_plate_number': results[(results['car_id'] == car_id) &
                                                             (results['licence_number_score'] == max_)]['licence_number'].iloc[0]}
    cap.set(cv2.CAP_PROP_POS_FRAMES, results[(results['car_id'] == car_id) &
                                             (results['licence_number_score'] == max_)]['frame_number'].iloc[0])
    ret, frame = cap.read()
    ...
    licence_plate[car_id]['licence_crop'] = licence_crop
...
while ret:
    ...
    if ret:
        ...
        for row_indx in range(len(df_)):
            ...
            # crop licence plate
            licence_crop = licence_plate[df_.iloc[row_indx]['car_id']]['licence_crop']
            ...
            try:
                ...
                (text_width, text_height), _ = cv2.getTextSize(
                    licence_plate[df_.iloc[row_indx]['car_id']]['licence_plate_number'],
